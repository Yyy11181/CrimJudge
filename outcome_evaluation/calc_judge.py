...
class MetricsCalculator:
    ...
    def load_data(self, file_path):
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            if "gen_ans" in data[0]:
                ...
                return {item['Fact'] : item['Full Document'] for _, item in enumerate(data)}

    def get_all_from_text(self, text):
        # print(text)
        return get_crime(text), calc_time_sum(text), get_penalcode_index_from_text(text)
    ...
    def calc_metrics(self):
        with open(r'evaluation_results.txt', 'w', encoding='utf-8') as f:
            # 写入文件头
            f.write("===== 新评估会话 =====\n")
            for exp_id, exp_ans in self.exp_data.items():
                gen_ans = self.gen_data[exp_id]

                exp_crime, exp_time, exp_penalcode_index = self.get_all_from_text(exp_ans)
                gen_crime, gen_time, gen_penalcode_index = self.get_all_from_text(gen_ans)
...
def main():
    ...
    parser.add_argument('--gen_file', type=str, default=r'data\单人多罪\judge\qwen235\output1.jsonl', help='Path to the input generated JSON file')
    parser.add_argument('--exp_file', type=str, default=r'data\单人多罪\judge\judge_final.jsonl', help='Path to the expected JSON file')
