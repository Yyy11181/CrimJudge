...
class MetricsCalculator:
    ...
    def calculate_recall_and_precision_article(self, expected, actual):
        expected_set = set(expected) #这是真实的
        actual_set = set(actual) #这是预测的
    ...
    def calc_metrics(self):
        with open(r'evaluation.txt', 'w', encoding='utf-8') as f:
            # 写入文件头
            f.write("===== 新评估会话 =====\n")
            for exp_id, exp_ans in self.exp_data.items():
                ...
                exp_crime, exp_time, exp_penalcode_index = exp_ans['accusation'], exp_ans['term_of_imprisonment']['imprisonment'], exp_ans['relevant_articles']

                crime_rec , crime_prec = self.calculate_recall_and_precision_article(exp_crime, gen_crime)
...
def main():
    ...
    parser.add_argument('--gen_file', type=str, default=r'data\单人多罪\cail\R1最终版_new.jsonl', help='Path to the input generated JSON file')
    parser.add_argument('--exp_file', type=str, default=r'data\单人多罪\cail\cail_final.jsonl', help='Path to the expected JSON file')
