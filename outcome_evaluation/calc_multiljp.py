...
class MetricsCalculator:
    def __init__(self, gen_file, exp_file):
        ...
        for key, value in self.gen_data.items():
            if value is None:
                print(f"Warning: gen_data key '{key}' has None value")
    ...
    def calc_time_sum(self,all_judge_time_str):
        time_sum = 0
    ...
    def calc_metrics(self):
        ...
        with open('comparison_results.txt', 'w', encoding='utf-8') as f:
            # 写入表头
            header = f"{'='*50}\n{'Exp ID':<15} {'Category':<20} {'Actual':<30} {'Predicted':<30}\n{'-'*50}\n"
            f.write(header)
            ...
            for exp_id, exp_ans in self.exp_data.items():
                ...
                exp_penalcode_index = []
                time = []
                for _, info in exp_ans['criminals_info'].items():
                    exp_crime.extend(info['accusations'])  # 真实罪名
                    exp_penalcode_index.extend(info['laws']) # 真实法条
                    time.append(info['term']) # 真实刑期
                exp_time = self.calc_time_sum(time)

                # 准备输出内容
                crime_line = f"{'Crime':<20} {str(exp_crime):<30} {str(gen_crime):<30}\n"
                code_line = f"{'Penal Code':<20} {str(exp_penalcode_index):<30} {str(gen_penalcode_index):<30}\n"
                time_line = f"{'Sentence Time':<20} {str(exp_time):<30} {str(gen_time):<30}\n"
                separator = f"{'-'*50}\n"
                ...
                penalcode_index_rec, penalcode_index_prec = self.calculate_recall_and_precision_article(exp_penalcode_index, gen_penalcode_index)
...
def main():
    ...
    parser.add_argument('--gen_file', type=str, default=r'data\多被告\multiljp\gemini-2.5\output_valid.jsonl', help='Path to the input generated JSON file')
    parser.add_argument('--exp_file', type=str, default=r'data\多被告\multiljp\final.jsonl', help='Path to the expected JSON file')
