...
def get_cause_names():
    cause_names = set()
    with open(r'outcome_evaluation\other\cause_codes.txt', 'r', encoding='utf8') as f:
        for line in f:
            l = line.strip()
            if l[-1] != '罪':
                print(l)
...
def get_crime(full_doc):
    ...
    if len(crime_list) == 0:
        ...
        #cail的时候，把里面的“罪”字去掉,cail,其余都不用去
    # for i in range(len(crime_list)):
    #     crime_list[i] = crime_list[i].replace('罪','')
    return crime_list
    

def main(): 
    file = r'multiple_persons_multiple_crime\deepseek-ai\DeepSeek-R1.json'
    ids = set()
